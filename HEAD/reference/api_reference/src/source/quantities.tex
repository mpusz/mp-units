...
\tcode{symbol_text<N, M>} is a structural type~(REFCPPX(temp.param)(term.structural.type)).
...
\begin{itemize}
\item
values of \tcode{char} are in the basic literal character set~(REFCPP(lex.charset)), and
\item
...
\pnum
\tcode{power} represents a power~(REFIEV(102-02-08))
...
\begin{itemize}
\item
\tcode{OneType} is the neutral element~(REFIEV(102-01-19)) of the operation, and
\item
\tcode{Pred} is a \oldconcept{BinaryTypeTrait}~(REFCPP(meta.rqmts))
...
\item
to $\textrm{a} \times 1/\textrm{b}^2$ where $\textrm{a} = \tcode{decltype(km)}$ and $\textrm{b} = \tcode{decltype(h)}$ (symbolic substitution)
...
for describing the reference of a quantity~(REFIEV(112-01-01)).
...
for defining the dimension of a quantity~(REFIEV(112-01-11)).
...
to define the dimension of a base quantity~(REFIEV(112-01-08)).
...
to represent the dimension of a derived quantity~(REFIEV(112-01-10)).
\begin{example}
\begin{codeblock}
...
\pnum
\tcode{dimension_one} represents the dimension of a quantity of dimension one~(REFIEV(112-01-13)).
...
for defining a quantity~(REFIEV(112-01-01)).
...
The identifier of \tcode{Q} represents its quantity name~(REFIEV(112-01-02)).
...
then \tcode{Q} is that base quantity~(REFIEV(112-01-08)).
If an argument is a quantity calculus~(REFIEV(112-01-30)) \placeholder{C},
then \tcode{Q} is implicitly convertible from \placeholder{C}.
If the first argument is a named quantity,
then \tcode{Q} is of its kind~(REFIEV(112-01-04)).
...
\pnum
\tcode{dimensionless} represents the base quantity of dimension one~(REFIEV(112-01-13)).
...
\pnum
\tcode{kind_of<Q>} represents a kind of quantity~(REFIEV(112-01-04)) \tcode{Q}.
...
let the kind of quantity~(REFIEV(112-01-04)) hierarchy of \tcode{q} be the tuple
...
for defining a unit of measurement~(REFIEV(112-01-14)).
...
used to represent the numerical value~(REFIEV(112-01-29)) of a unit
with support for powers~(REFIEV(102-02-08)) of real numbers~(REFIEV(102-02-05)).
...
A specialization of \tcode{mag_constant} represents a real number~(REFIEV(102-02-05)).
...
Such specializations shall be usable in constant expressions~(REFCPP(expr.const))
...
\pnum
\exposid{canonical-unit} represents a unit expressed in terms of base units~(REFIEV(112-01-18)).
\begin{note}
...
its unit name~(REFIEV(112-01-15))
or special unit name~(REFIEV(112-01-16)).
\tcode{Symbol} is its unit symbol~(REFIEV(112-01-17)).
...
without a unit prefix~(REFIEV(112-01-26)).
...
The first and fourth signatures with a kind of quantity~(REFIEV(112-01-04)) \placeholder{Q}
...
} hertz;  // $\textrm{Hz}$ can't measure becquerel, activity,
          // or any other quantity with dimension $\textrm{T}^{-1}$
...
\pnum
\tcode{prefixed_unit<Symbol, M, U>} represents the unit \tcode{U} with a unit prefix~(REFIEV(112-01-26)).
...
to encapsulate a conversion factor between units~(REFIEV(112-01-33))
common to the operands of quantity addition.
\begin{example}
The result of \tcode{1 * km + 1 * mi}
has a common unit $[8/125] \textrm{ m}$
...
to represent a derived unit~(REFIEV(112-01-19)).
\begin{example}
\begin{codeblock}
using namespace si::unit_symbols;
...
\pnum
\tcode{one} represents the base unit~(REFIEV(112-01-18)) of a quantity of dimension one~(REFIEV(112-01-13)).
...
represents the reference of a quantity~(REFIEV(112-01-01)).
...
\pnum
\tcode{reference<Q, U>} represents the reference of a quantity~(REFIEV(112-01-01)).
The unit of measurement \tcode{U}~(REFIEV(112-01-14))
is used to measure a value of the quantity \tcode{Q}~(REFIEV(112-01-28)).
\begin{note}
...
used to constrain the numerical value of a quantity~(REFIEV(112-01-29)).
...
\begin{itemdescr}
\pnum
Some quantities are defined as having a numerical value~(REFIEV(112-01-29)) of a specific set~(REFIEV(102-01-02)).
...
\pnum
The requirements on \tcode{std::chrono::duration_values<Rep>}~(REFCPP(time.traits.duration.values))
...
\pnum
\remarks
The value returned shall be the neutral element for multiplication~(REFIEV(102-01-19)).
\end{itemdescr}
...
\pnum
The name \tcode{mp_units::real} denotes a customization point object~(REFCPP(customization.point.object)).
...
where the meaning of \tcode{real} is established as-if by performing argument-dependent lookup only~(REFCPP(basic.lookup.argdep)),
...
\pnum
The name \tcode{mp_units::imag} denotes a customization point object~(REFCPP(customization.point.object)).
...
where the meaning of \tcode{imag} is established as-if by performing argument-dependent lookup only~(REFCPP(basic.lookup.argdep)),
...
\pnum
The name \tcode{mp_units::modulus} denotes a customization point object~(REFCPP(customization.point.object)).
...
where the meaning of \tcode{modulus} is established as-if by performing argument-dependent lookup only~(REFCPP(basic.lookup.argdep)),
...
where the meaning of \tcode{abs} is established as-if by performing argument-dependent lookup only~(REFCPP(basic.lookup.argdep)),
then \tcode{mp_units::modulus(E)} is expression-equivalent to that expression.
\item
...
\pnum
The name \tcode{mp_units::magnitude} denotes a customization point object~(REFCPP(customization.point.object)).
...
where the meaning of \tcode{magnitude} is established as-if by performing argument-dependent lookup only~(REFCPP(basic.lookup.argdep)),
...
it represents the numerical value of a quantity~(REFIEV(112-01-29)).
...
that represents the value of a quantity~(REFIEV(112-01-28))
that is an element of a vector space~(REFIEV(102-03-01,102-03-04)).
...
\begin{itemize}
\item
\tcode{Traits<Q>::to_numerical_value(qty)} returns the numerical value~(REFIEV(112-01-29)) of \tcode{qty}.
\item
...
\pnum
\tcode{quantity<R, Rep>} is a structural type~(REFCPPX(temp.param)(term.structural.type))
...
\item
Otherwise, returns \tcode{is_\placeholdernc{F}(q <=> zero())}~(REFCPP(compare.syn)).
\end{itemize}
...
that represents the value of a quantity~(REFIEV(112-01-28))
that is an element of an affine space~(REFIEV(102-03-02,102-04-01)).
...
An \defn{origin} is a point from which measurements~(REFIEV(112-04-01)) take place.
...
of a subspace~(REFIEV(102-03-03)).
...
\pnum
\tcode{quantity_point<R, PO, Rep>} is a structural type~(REFCPPX(temp.param)(term.structural.type))
if \tcode{Rep} is a structural type.
