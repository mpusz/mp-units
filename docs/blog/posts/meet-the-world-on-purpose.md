...
prose that does not survive a second read. It took a language model ninety seconds to
produce, and it will take you an hour to review. Now picture ten of them by morning. This
is the new maintainer reality, and pretending it is not happening is not a plan.
...
So the answer is not a ban, it is a written policy, in `CONTRIBUTING.md`, where a contributor
reads it before opening a pull request. It does not need to be long. It needs to name the
...
- **You are the author.** However much a model or an agent wrote, you must understand and
  be able to defend every line. A blind refactor nobody can explain is not ready, however
...
five-minute path to contribute, does not spread on its own. Discovery, back in the first post,
gets you found by people already searching. Growth means reaching the ones who are not looking
yet. That is evangelism, and for a library author it is not marketing gloss, it is doing the
...
steady standardization work, a regular blog, and a presence on r/cpp. mp-units is not a quiet
...
is not for lack of good people. mp-units has had real co-maintainers,
...
cannot automate it or gate it in CI, and it is the part I have least figured out.
