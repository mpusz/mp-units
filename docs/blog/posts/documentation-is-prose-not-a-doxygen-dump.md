...
better than most, which is why technical brilliance on its own doesn't help here. If the
...
I want to be fair to Doxygen, because it is often cast as the villain and it isn't. An
...
it to someone learning the language, and it teaches them nothing, because they don't yet
know what to look up. "Sets the width. Parameter: the width to set" is a definition that
doesn't help anyone who did not already understand the function.
...
one document can't serve all four.
...
describes, and a feature isn't finished until its documentation is written.
...
configuring CMake, and resolving dependencies, most readers won't bother.

The cost isn't only convenience. Typing an example builds muscle memory, and you learn the
...
**mp-units** doesn't yet follow all of the advice in this article, so here is the history,
...
It isn't a documentation pattern to copy. Standardese is written to remove ambiguity for
