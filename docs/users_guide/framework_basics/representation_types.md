...
You usually do not need to wire up the customization points described above yourself for
a mainstream third-party library. **mp-units** ships opt-in integration plugins — currently
for [Eigen](https://eigen.tuxfamily.org), [GLM](https://github.com/g-truc/glm), and
[Blaze](https://bitbucket.org/blaze-lib/blaze) — so their vector and matrix types can be
used **directly** as vector/tensor representations. The
[Using a Linear Algebra Library](../../how_to_guides/integration/using_linear_algebra_libraries.md)
how-to guide is the single home for the shipped-plugin list, the recipe for adapting a
library that has no plugin, and the caveats (expression templates, the unsupported Armadillo
`operator==`, and the V2 vector-operation result-type limitation).
